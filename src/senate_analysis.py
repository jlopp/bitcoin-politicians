...
file_handler = logging.FileHandler('analysis_logs.txt')
...
def analyze_text_chunk(text_chunk, prompt):
    """
    Analyze a chunk of text using GPT-4.
...
def quick_vision_test(file_path: str, file_type: str = 'pdf') -> dict:
    """
    Analyze disclosure file based on specified type using OpenAI's GPT-4 model.
...
def analyze_reports(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): DataFrame with 'Filename' column.
    ...
    for idx, row in df.iterrows():
        ...
        if pd.notna(filename) and filename.strip():
            logger.info(f"Processing report {idx + 1}/{len(df)}: {filename}")
            ...
            logger.warning(f"No valid filename found for row {idx + 1}. Skipping.")
...
def run_analysis():
    ...
    """
    input_csv = 'senate_disclosure_reports.csv'
    output_csv = 'senate_disclosure_reports_with_analysis.csv'
